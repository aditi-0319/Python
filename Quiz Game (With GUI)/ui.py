...
class QuizInterface:
    def __init__(self, quiz: QuizBrain):
        ...
        for i, genre in enumerate(genres):
            self.button = Button(text=genre, width=20, command=lambda category=i + 1: self.select_genre(category))
            self.button.grid(row=1 + i, column=0)
            self.genre_buttons.append(self.button)
    ...
    def select_genre(self, category: int):
        ...
        number = min(len(question_list), int(self.spinbox.get()))
